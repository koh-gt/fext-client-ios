...
#      which is exactly what AltStore / SideStore / Sideloadly do).
#
#  Env:
...
directly — re-sign it with your own Apple ID using
[SideStore](https://sidestore.io), [AltStore](https://altstore.io) or
[Sideloadly](https://sideloadly.io). A free Apple ID gives a 7-day install; the
sideloading app refreshes it. See the README "Install on your iPhone in 10
minutes" guide.'
    PRERELEASE_ARG="--prerelease=true"
fi

NOTES="**${TITLE}** — FEXT for iOS (v7 client).
